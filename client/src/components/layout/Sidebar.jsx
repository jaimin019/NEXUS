...
export default function Sidebar() {
  ...
  const { sidebarCollapsed, toggleSidebar, activePage, setActivePage, activeAlerts } = useNexusStore();
    ...
    >
          ...
          return (
            <div key={item.id} className="relative group">
              <button
                ...
                `}
              >
                <Icon className={`w-5 h-5 flex-shrink-0 ${active ? 'text-nexus-primary' : ''}`} />
                <AnimatePresence>
                  {!sidebarCollapsed && (
                    <motion.span
                      initial={{ opacity: 0 }}
            ...
            <AnimatePresence>
              {!sidebarCollapsed ? (
                ...
                >
                  ...
                  <span className="text-xs text-red-300 font-medium">{activeAlerts.length} Active Alerts</span>
                </motion.div>
              ) : (
                <motion.div
                  initial={{ opacity: 0 }}
      ...
      <div className="flex-shrink-0 p-3 border-t border-nexus-border">
        <button
          ...
          className="w-full flex items-center justify-center p-2 rounded-lg text-nexus-muted
